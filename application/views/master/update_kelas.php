    ...
    <div class="col-md-8 offset-md-2 widget-holder">
      <div class="widget-bg">
        <div class="widget-body clearfix">
          <h5 class="box-title mr-b-0">Update Kelas</h5>
          <p class="text-muted">Pastikan data kelas benar</p>
          <form id="form-pendaftaran">
            <div class="form-group">
              <label for="l38">NAMA KELAS</label>
              <input class="form-control" name="NAMA_KELAS" type="text" placeholder="Masukkan Nama Kelas" value="<?= $kelas->NAMA_KELAS ?>">
            </div>
            <div class="row">
              <div class="col-lg-12">
                <div class="form-group">
                  ...
                  <select class="form-control" name="GURU" data-toggle="select2">
                    ...
                    <?php if ($guru !== null) : foreach ($guru as $key) : ?>
                      <option value="<?= $key->NIP; ?>" <?= $key->NIP == $kelas->NIP ? 'selected' : null ?>><?= $key->NAMA_LENGKAP_GURU; ?></option>
                    <?php endforeach;
                  endif; ?>
                </select>
              </div>
...
<script>
  $("#form-pendaftaran").on('submit', (e) => {
    ...
    $.ajax({
      type: "POST",
      url: "<?= base_url(); ?>api/update/kelas/<?= $kelas->ID_KELAS ?>",
